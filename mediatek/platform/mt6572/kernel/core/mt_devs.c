...
#include <linux/proc_fs.h>  //zhoulidong add
...
#if 0// MT6572 camera not using SYSRAM
/*=======================================================================*/
/* MT6572 SYSRAM                                                         */
/*=======================================================================*/
...
struct platform_device rpi1040_gsensor = {
	.name	       = "rpi1040_gsensor",
	.id            = -1,
};
struct platform_device lis3dh_gsensor = {
	.name	       = "lis3dh_gsensor",
	.id            = -1,
};
...
// zhoulidong add
char rgk_mem_name[50]= {0};  // zhoulidong add

static int rgk_read_mem_info(char *page, char **start, off_t off,int count,
			  int *eof, void *data){
	ssize_t len = 0;
	if(off > 0)
	{
...
int rgk_creat_proc_mem_info(void)
{
	...
	mem_info_entry = create_proc_entry("rgk_memInfo", 0444, NULL);
	if (mem_info_entry)
	{
		mem_info_entry->read_proc = rgk_read_mem_info;
		mem_info_entry->write_proc = NULL;
	}
...
__init int mt_board_init(void)
{
    ...
    rgk_creat_proc_mem_info(); //zhoulidong add
...
// [MT6572] Need Porting
  ...
  if(retval != 0)
  {
	...
	retval = platform_device_register(&lis3dh_gsensor);
		printk("lis3dh_gsensor device!");
	retval = platform_device_register(&rpi1040_gsensor);
		printk("rpi1040_gsensor device!");
	if (retval != 0)
		return retval;
#endif
