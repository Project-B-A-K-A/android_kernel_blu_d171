...
void accdet_auxadc_switch(int enable)
{
   if (enable) {
	 ...
	 pmic_pwrap_write(ACCDET_RSV, ACCDET_1V9_ON);
     ACCDET_DEBUG("ACCDET enable switch in 1.9v mode \n");
	#else
	 pmic_pwrap_write(ACCDET_RSV, ACCDET_2V8_ON);
	 ACCDET_DEBUG("ACCDET enable switch in 2.8v mode \n");
	#endif
   }else {
   
   	#ifndef ACCDET_28V_MODE
	 pmic_pwrap_write(ACCDET_RSV, ACCDET_1V9_OFF);
     ACCDET_DEBUG("ACCDET diable switch in 1.9v mode \n");
	#else
	 pmic_pwrap_write(ACCDET_RSV, ACCDET_2V8_OFF);
...
void inline disable_accdet(void)
{
	...
	irq_temp = irq_temp & (~ACCDET_IRQ_CLR);
...
static void disable_micbias_callback(struct work_struct *work)
{
        ...
        if(cable_type == HEADSET_NO_MIC) {
			   ...
			   ACCDET_DEBUG("[Accdet] cable_pin_recognition = %d\n", cable_pin_recognition);
				pmic_pwrap_write(ACCDET_WIDTH, cust_headset_settings.pwm_width);
    			pmic_pwrap_write(ACCDET_THRESH, cust_headset_settings.pwm_thresh);
			#endif
                // setting pwm idle;
   ...
   //KE under fastly plug in and plug out
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		//micbias always on during detected PIN recognition
		  pmic_pwrap_write(ACCDET_WIDTH, cust_headset_settings.pwm_width);
    	  pmic_pwrap_write(ACCDET_THRESH, cust_headset_settings.pwm_width);
...
int getMicBiasVoltage(void)
{
	int cali_voltage=0;
	accdet_auxadc_switch(1);

	/*
	cali_voltage =PMIC_IMM_GetOneChannelValue(0,1,1);
	ACCDET_DEBUG("[Accdet]adc ch0 = %d mv\n", cali_voltage);
	cali_voltage =PMIC_IMM_GetOneChannelValue(1,1,1);
	ACCDET_DEBUG("[Accdet]adc ch1 = %d mv\n", cali_voltage);
	cali_voltage =PMIC_IMM_GetOneChannelValue(2,1,1);
	ACCDET_DEBUG("[Accdet]adc ch2 = %d mv\n", cali_voltage);
	cali_voltage =PMIC_IMM_GetOneChannelValue(3,1,1);
	ACCDET_DEBUG("[Accdet]adc ch3 = %d mv\n", cali_voltage);
	cali_voltage =PMIC_IMM_GetOneChannelValue(4,1,1);
	ACCDET_DEBUG("[Accdet]adc ch4 = %d mv\n", cali_voltage);
	cali_voltage =PMIC_IMM_GetOneChannelValue(5,1,1);
	ACCDET_DEBUG("[Accdet]adc ch5 = %d mv\n", cali_voltage);
	cali_voltage =PMIC_IMM_GetOneChannelValue(6,1,1);
	ACCDET_DEBUG("[Accdet]adc ch6 = %d mv\n", cali_voltage);
	cali_voltage =PMIC_IMM_GetOneChannelValue(7,1,1);
	ACCDET_DEBUG("[Accdet]adc ch7 = %d mv\n", cali_voltage);
	*/
	
	cali_voltage = PMIC_IMM_GetOneChannelValue(MULTIKEY_ADC_CHANNEL,1,1);
	ACCDET_DEBUG("[Accdet]adc ch8 = %d mv\n", cali_voltage);
...
static int multi_key_detection(void)
{
	...
	cali_voltage = getMicBiasVoltage();
	...
	while(index++ < count)
	{
		...
		cali_voltage = getMicBiasVoltage();
		ACCDET_DEBUG("[Accdet]adc channel:%d in while loop [%d]= %d mv\n",MULTIKEY_ADC_CHANNEL ,index, cali_voltage);
...
static inline void clear_accdet_interrupt(void)
{
	...
	pmic_pwrap_write(ACCDET_IRQ_STS, (ACCDET_IRQ_CLR));
	//pmic_pwrap_write(INT_CON_ACCDET_CLR, (RG_ACCDET_IRQ_CLR));
...
*/
        ...
        if(PLUG_OUT !=accdet_status)
        {
            ...
            {
                ACCDET_DEBUG("[Accdet][key] answer call!\n");
            ...
            {
                                ACCDET_DEBUG("[Accdet][key] PLAY_PAUSE !\n");
...
// next, previous, volumeup, volumedown send key in multi_key_detection()
            ...
            {
                                ACCDET_DEBUG("[Accdet][key] NEXT ..\n");
                      
            }
            if(ACC_MEDIA_PREVIOUS == accdet_key_event)
            {
                                ACCDET_DEBUG("[Accdet][key] PREVIOUS..\n");
                           
            }
	      if(ACC_VOLUMEUP== accdet_key_event)
            {
                                ACCDET_DEBUG("[Accdet][key] VOLUMUP ..\n");
                           
            }
	       if(ACC_VOLUMEDOWN== accdet_key_event)
            {
                                ACCDET_DEBUG("[Accdet][key] VOLUMDOWN..\n");
		...
		if(current_status == 0)
		{
			...
			current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0)>>6); //A=bit1; B=bit0
			 if (current_status == 0 && show_icon_delay != 0)
			 {
					...
					pin_adc_value = getMicBiasVoltage();
...
#ifdef ACCDET_PIN_RECOGNIZATION
			pmic_pwrap_write(ACCDET_WIDTH, REGISTER_VALUE(cust_headset_settings.pwm_width));
			pmic_pwrap_write(ACCDET_THRESH, REGISTER_VALUE(cust_headset_settings.pwm_thresh));
#endif
			//#ifdef ACCDET_LOW_POWER
...
void accdet_work_callback(struct work_struct *work)
{
	...
	mutex_lock(&accdet_eint_irq_sync_mutex);
    if(1 == eint_accdet_sync_flag) {
              switch_set_state((struct switch_dev *)&accdet_data, 1);
		msleep(100);
		switch_set_state((struct switch_dev *)&accdet_data, cable_type);
    }else {
...
{ 
    ...
    pmic_pwrap_write(TOP_CKPDN_CLR, 0x0200); 
    ...
    pmic_pwrap_write(ACCDET_DEBOUNCE3, cust_headset_settings.debounce3);	
   #endif
    pmic_pwrap_write(ACCDET_IRQ_STS, pmic_pwrap_read(ACCDET_IRQ_STS)&(~ACCDET_IRQ_CLR));
...
static int dump_register(void)
{
	...
	ACCDET_DEBUG("[0x102] = 0x%x\n", pmic_pwrap_read(0x102));
	ACCDET_DEBUG("[0x160] = 0x%x  [0x166] = 0x%x\n", pmic_pwrap_read(0x160), pmic_pwrap_read(0x166));
	ACCDET_DEBUG("[0x172] = 0x%x  [0x174] = 0x%x\n", pmic_pwrap_read(0x172), pmic_pwrap_read(0x174));
	ACCDET_DEBUG("[0x77A] = 0x%x  [0x77C] = 0x%x\n", pmic_pwrap_read(0x77a), pmic_pwrap_read(0x77c));
	ACCDET_DEBUG("[0x77E] = 0x%x  [0x780] = 0x%x\n", pmic_pwrap_read(0x77e), pmic_pwrap_read(0x780));
	ACCDET_DEBUG("[0x782] = 0x%x  [0x784] = 0x%x\n", pmic_pwrap_read(0x782), pmic_pwrap_read(0x784));
	ACCDET_DEBUG("[0x786] = 0x%x  [0x788] = 0x%x\n", pmic_pwrap_read(0x786), pmic_pwrap_read(0x788));
	ACCDET_DEBUG("[0x78A] = 0x%x  [0x78C] = 0x%x\n", pmic_pwrap_read(0x78a), pmic_pwrap_read(0x78c));
	ACCDET_DEBUG("[0x78e] = 0x%x  [0x790] = 0x%x\n", pmic_pwrap_read(0x78e), pmic_pwrap_read(0x790));	
	ACCDET_DEBUG("[0x792] = 0x%x  [0x794] = 0x%x\n", pmic_pwrap_read(0x792), pmic_pwrap_read(0x794));
	ACCDET_DEBUG("[0x796] = 0x%x  [0x798] = 0x%x\n", pmic_pwrap_read(0x796), pmic_pwrap_read(0x798));
	ACCDET_DEBUG("[0x79a] = 0x%x \n", pmic_pwrap_read(0x79a));	
