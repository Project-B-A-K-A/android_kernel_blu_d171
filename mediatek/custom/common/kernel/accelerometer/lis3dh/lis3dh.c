...
static DEFINE_MUTEX(lis3dh_mutex);
...
static int LIS3DH_SetDataResolution(struct lis3dh_i2c_data *obj)
{
	...
	if((err = lis_i2c_read_block(obj->client, LIS3DH_REG_CTL_REG4, &dat,0x01)))
	{
...
*/
    // first clear reg1
    res = hwmsen_write_byte(client,LIS3DH_REG_CTL_REG1,0x0f);
	if(res != LIS3DH_SUCCESS)
	{
		return res;
	}
...
/*----------------------------------------------------------------------------*/
int lis3dh_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static int lis3dh_suspend(struct i2c_client *client, pm_message_t msg) 
{
	...
	{   
		...
		//read old data
		if ((err = lis_i2c_read_block(client, LIS3DH_REG_CTL_REG1, &dat, 0x01))) 
		{
           GSE_ERR("write data format fail!!\n");
           return err;
        }
...
struct device_driver *platform_gsensor_driver = NULL ;
int lis3dh_exist = 0;
#endif
static int lis3dh_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#ifdef GSENSOR_BE_COMPATIABLE 
        lis3dh_exist = 1;
	 platform_gsensor_driver = driver_find("gsensor",lis3dh_gsensor_driver.driver.bus);
  	 if(platform_gsensor_driver ==NULL)
    	{
		GSE_ERR("platform_gsensor_driver not registed yet\n");
		goto exit_misc_device_register_failed;
	}
	if((err = lis3dh_create_attr(platform_gsensor_driver)))
	#else
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = lis3dh_gsensor_operate;
...
static int lis3dh_i2c_remove(struct i2c_client *client)
{
	int err = 0;	
	#ifdef GSENSOR_BE_COMPATIABLE 
	if((err = lis3dh_delete_attr(platform_gsensor_driver)))
	#else
