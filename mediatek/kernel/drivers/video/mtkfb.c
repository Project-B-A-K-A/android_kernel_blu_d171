...
static int esd_recovery_kthread(void *data)
{
    ...
    for( ;; ) {
        ...
        msleep(1000);       //2s  zhoulidong modify 2s-1s
...
{
#if defined(MTK_M4U_SUPPORT)
    MTKFB_INFO("[FB driver] call _m4u_lcd_init()\n");
    ...
    MTKFB_INFO("[m4u_lcd_init] VA = 0x%x, MVA = 0x%x\n",fb_va_m4u, fb_mva_m4u);
#else
    MTKFB_INFO("[FB driver] call _m4u_lcd_init() but this function not implement for MTKFB not use M4U\n");
#endif
...
void mtkfb_m4u_switch(bool enable)
{
    ...
    MTKFB_FUNC();
    if (down_interruptible(&sem_flipping)) {
        MTKFB_WRAN("[FB Driver] can't get semaphore in mtkfb_m4u_switch()\n");
        return;
    }
    sem_flipping_cnt--;
    if (down_interruptible(&sem_early_suspend)) {
    ...
    up(&sem_flipping);
#else
    MTKFB_INFO("[FB driver] call mtkfb_m4u_switch() but this function not implement for MTKFB not use M4U\n");
#endif
    
    return;
}
EXPORT_SYMBOL(mtkfb_m4u_switch);
...
void mtkfb_m4u_dump(void)
{
    ...
    DISP_DumpM4U();
#else
    MTKFB_INFO("[FB driver] call mtkfb_m4u_dump() but this function not implement for MTKFB not use M4U\n");
#endif
    
    return;
}
EXPORT_SYMBOL(mtkfb_m4u_dump);
    ...
    while(c != NULL){
        if(c->file_addr == file_addr){
            ...
            DISP_DeallocMva(c->buffer.src_vir_addr, c->src_mva, c->buffer.size);
            {
                ...
                MTKFB_INFO("[mtkfb_release] before delete a node,list node Addr:\n");
                
                while(a)
                {
                    MTKFB_INFO("0x%x,  ", (unsigned int)a);
                    a = a->next;
                }
            }
            n->next = c->next;
            vfree(c);
            {
                ...
                while(b)
                {
                    MTKFB_INFO("0x%x,  ", (unsigned int)b);
                    b = b->next;
                    MTKFB_INFO("\n");
                }
            }
...
static int mtkfb_release(struct file *file, struct fb_info *info, int user)
{
...
#if defined (MTK_M4U_SUPPORT)
    {
        ...
        if(mtkfb_search_overlayList_ex(overlay_buffer_head, (void*)file)){
            MTKFB_INFO("disable all video layer\n");
...
static BOOL MTKFB_SearchMVA(UINT32 mva)
{
    ...
    if(c == NULL){
        MTKFB_WRAN("[LCD Warning] mva = 0x%x, buffer list is NULL\n",mva);
        return FALSE;
    }
    while(mva != c->src_mva){
        c = c->next;
        if(!c){
            MTKFB_WRAN("[LCD Warning] mva = 0x%x,Can't find matched VA in buffer list\n",mva);
            ret = FALSE;
            break;
        }
    }
...
static int mtkfb_set_par(struct fb_info *fbi)
{
    ...
    if(is_ipoh_bootup&& DISP_IsDSIVdoMode())
    {
...
static struct fb_overlay_buffer_list* mtkfb_search_overlayList(struct fb_overlay_buffer_list* head, unsigned int va)
{
    ...
    while(va != c->buffer.src_vir_addr || c->pid != current->tgid){
        ...
        if(!c){
            ...
            p = head->next;
            //			ASSERT(p);
            MTKFB_INFO("[FB driver] current va = 0x%x, buffer_list va:\n",va);
            while(p){
                MTKFB_INFO("0x%x\n", p->buffer.src_vir_addr);
                p = p->next;
            }
            MTKFB_WRAN("[FB driver] ERROR: Can't find matched VA in buffer list\n");
            break;
        }
    }
    return c;
...
static int mtkfb_set_overlay_layer(struct fb_info *info, struct fb_overlay_layer* layerInfo)
{
    ...
    */
    ...
    if((layerInfo->layer_enable != 0) && (layerInfo->layer_id == 3) && (layerInfo->src_fmt != MTK_FB_FORMAT_ARGB8888))
    {
        printk("ERROR!!!Layer 3 format error!!!\n");
    }

    MTKFB_INFO("[FB Driver] mtkfb_set_overlay_layer():layer id = %u, layer en = %u, src format = 0x%x, direct link: %u, src vir addr = 0x%x, src phy addr = 0x%x, src pitch=%u, src xoff=%u, src yoff=%u, src w=%u, src h=%u\n",
                        layerInfo->layer_id,
    ...
    MTKFB_INFO("[FB Driver] mtkfb_set_overlay_layer():target xoff=%u, target yoff=%u, target w=%u, target h=%u\n",
    ...
    if(FB_LAYER != id && (FB_LAYER + 1) != id){
        ...
        if(u4OvlPhyAddr == 0)
        {
            MTKFB_WRAN("[FB Error]overlay_va should not be 0x00000000\n");
...
*/
    ...
    if(FB_LAYER == id){
        ...
        if(!is_pmem_range(u4OvlPhyAddr, layerpitch * layerInfo->src_width * layerInfo->src_height)){
            MTKFB_WRAN("error: M4U is not supported but non-PMEM is set to video-layer\n");
            ASSERT(0);
        }
    }
...
#if defined(DITHERING_SUPPORT)
    {
        ...
        if(ditherbpp < layerbpp)
        {
            if(ditherbpp == 16)
            {
                ...
                {
                    MTKFB_WRAN("ERROR, error dithring bpp settings\n");
                }
            }
            ...
            {
                MTKFB_WRAN("ERROR, error dithering bpp settings, diterbpp = %d\n",ditherbpp);
                ASSERT(0);
            }
            ...
            if(ditherenabled)
            {
                ...
                if(FB_LAYER == id){
                    ...
                    MTKFB_INFO("[FB driver] dither enabled:%d, dither bit(%d,%d,%d)\n", fblayer_dither_needed, dbr_backup, dbg_backup, dbb_backup);
                }
            }
        }
        else
        ...
        // debug message
        #if 0
        MTKFB_INFO("[%s] layer %d, ditherbpp=%d, layerbpp=%d\n", __func__, id, ditherbpp, layerbpp);
        MTKFB_INFO("[%s] dithering enabled: %d, dbr=%d, dbg=%d, dbb=%d\n", __func__, ditherenabled, dbr, dbg, dbb);
        #endif
    }
#endif
...
static int mtkfb_get_video_layer(struct fb_info *info, struct fb_overlay_layer *layerInfo)
{
    ...
    if(layerInfo->layer_enable == 0){
        MTKFB_WRAN("[FB Driver] Video Layer not enable when mtkfb_get_video_layer()\n");
        return -1;
    }
...
static int mtkfb_capture_videobuffer(struct fb_info *info, unsigned int pvbuf)
{
    ...
    if(0 == video_layerInfo.layer_enable){// video layer not enable
        MTKFB_WRAN("[FB Driver] Video Layer not enable when mtkfb_capture_videobuffer()\n");
        return -1;
    }
...
EXIT:
    if (is_early_suspended) {
        ...
        disp_path_clock_off("reg_non_backup");
#endif
    }
...
static int mtkfb_set_s3d_ftm(struct fb_info *info, unsigned int mode)
{
    ...
    if (!framebuffer)
    {
        MTKFB_WRAN("[FB Driver] can't allocate memory in mtkfb_set_s3d_ftm()\n");
        return -ERESTARTSYS;
    }
    ...
    if (down_interruptible(&sem_flipping)) {
        MTKFB_WRAN("[FB Driver] can't get semaphore in mtkfb_set_backlight_pwm()\n");
        return -ERESTARTSYS;
    }
    sem_flipping_cnt--;
...
static int mtkfb_ioctl(struct file *file, struct fb_info *info, unsigned int cmd, unsigned long arg)
{
    ...
    {
        MTKFB_INFO(" mtkfb_ioctl():MTKFB_GET_VIDEO_LAYER\n");
    ...
    {
        unsigned int pbuf = 0;
        MTKFB_INFO(" mtkfb_ioctl(): MTKFB_CAPTURE_VIDEOBUFFER\n");
        if (copy_from_user(&pbuf, (void __user *)arg, sizeof(pbuf))) 
        {
            MTKFB_WRAN("[FB]: copy_from_user failed! line:%d \n", __LINE__);
            r = -EFAULT;
...
*/
...
#else
        MTKFB_WRAN("[FB]: not enable m4u! line:%d \n", __LINE__);
#endif
		return (r);
    }
	...
	case MTKFB_UNREGISTER_OVERLAYBUFFER:
    {
		...
		else
		{
			...
			{
				MTKFB_WRAN("[mtkfb_ioctl Error]MTKFB_UNREGISTER_OVERLAYBUFFER VA should not be 0x00000000\n");
				return (r);
			}
...
//				ASSERT(n); //n is NULL, but not find matched buffer
	...
	{
		MTKFB_INFO("[MTKFB] Set Composing 3D: %lu\n", arg);
	...
	{
		MTKFB_INFO("[MTKFB] Set S3D Factory Mode: %lu\n", arg);
...
////////////////////////////////////////////FM De-sense
	...
	{
		unsigned long freq;
		MTKFB_INFO("[MTKFB] FM set new channel\n");
		if(DISP_STATUS_OK != DISP_FMDesense_Query()){
		    MTKFB_WRAN("[FB]: this project not support FM De-sense\n");
			return -EFAULT;
		}
		...
		if (copy_from_user(&freq, (void __user *)arg, sizeof(freq))) {
            ...
            MTKFB_INFO("[MTKFB] FM channel freq is %ld\n", freq);
	...
	{
		MTKFB_INFO("[MTKFB] FM be turned off\n");
		if(DISP_STATUS_OK != DISP_FMDesense_Query()){
		    MTKFB_WRAN("[FB]: this project not support FM De-sense\n");
			return -EFAULT;
		}
	...
	case MTKFB_BOOTANIMATION:// this interface only for 6573 bootanimation
	{
		...
		if (down_interruptible(&sem_early_suspend)) 
		{
        	MTKFB_WRAN("[FB Driver] can't get semaphore in mtkfb_bootanimation()\n");
        	return -ERESTARTSYS;
    	}
...
unsigned int mtkfb_fm_auto_test()
{
	...
	result = DISP_LCMTest(fbVirAddr, fb_pa, ALIGN_TO(DISP_GetScreenWidth(),32)*DISP_GetScreenHeight());
	printk("mtkfb_fm_auto_test LCM result = %d\n",result);
	up(&sem_flipping);
#else
//	mtkfb_fbi->var.yres = 0;
//	mtkfb_fbinfo_modify(mtkfb_fbi);
                      ...
                      u32 copyHeight)
{
    ...
    {
        MTKFB_INFO("un-supported bpp in copy_rect(), srcBpp: %d --> dstBpp: %d\n",
               srcBpp, dstBpp);
...
static int mtkfb_probe(struct device *dev)
{
    ...
    fbdev->fb_size_in_byte = MTK_FB_SIZEV;
    {
        ...
        {
            printk("Boot logo unavailable, clear framebuffer\n");
            memset(fbdev->fb_va_base, 0, (res->end - res->start + 1));
        }
    }
#if defined(MTK_M4U_SUPPORT)
...
static void mtkfb_shutdown(struct device *pdev)
{
    ...
    // TO BE ENABLED after mt65xx_leds_brightness_set is provided.
...
//    UINT32 fbVirAddr = (UINT32)fbdev->fb_va_base + mtkfb_fbi->var.yoffset * mtkfb_fbi->fix.line_length;
    ...
    if(is_early_suspended){
        ...
        sem_early_suspend_cnt++;
        up(&sem_early_suspend);
        MTKFB_INFO("[FB driver] has been suspended\n");
        mutex_unlock(&ScreenCaptureMutex);
        return;
    }
...
//	memset(fbVirAddr, 0, MTK_FB_SIZE);
    ...
    {
        mtkfb_clear_lcm();
        msleep(100);// JEEY-149  pengjinlong motify
    }
...
//zhoulidong add
static int rgk_read_lcm_info(char *page, char **start, off_t off,int count,
			  int *eof, void *data){
	ssize_t len = 0;
	if(off > 0)
	{
...
int rgk_creat_proc_lcm_info(void)
{

	struct proc_dir_entry *lcm_info_entry = NULL;

	rgk_lcm_command_line();
	
	lcm_info_entry = create_proc_entry("rgk_lcdInfo", 0444, NULL);
	if (lcm_info_entry)
	{
			lcm_info_entry->read_proc = rgk_read_lcm_info;
			lcm_info_entry->write_proc = NULL;
	}
...
int __init mtkfb_init(void)
{
    ...
    DBG_Init();
   rgk_creat_proc_lcm_info(); // zhoulidong add
exit:
    MSG_FUNC_LEAVE();
    return r;
}
