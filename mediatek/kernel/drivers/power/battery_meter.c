...
int BattVoltToTemp(int dwVolt)
{
	...
	/*JELY-73 pengjinlong add this for avoid shutdown devices when the devices poweron without battery (start)*/
	if (dwVolt >=((RBAT_PULL_UP_VOLT*RBAT_PULL_DOWN_R)/(RBAT_PULL_DOWN_R + RBAT_PULL_UP_R) - 10) )
	{
	 	bm_print(BAT_LOG_FULL, "[BattVoltToTemp]return -20 \n");
		return -20;
	}
	/*(end)*/
...
void dod_init(void)
{
    ...
    #endif
//pengjinlong change (DELL-888)	(start)
//pengjinlong  add for recovery mode JELY-1572(start)
    if(g_rtc_fg_soc >= gFG_capacity_by_v)
    {
        if(((g_rtc_fg_soc != 0) && ((g_rtc_fg_soc-gFG_capacity_by_v) < 40) &&(( gFG_capacity_by_v > 1 || bat_is_charger_exist() == KAL_TRUE)))
			|| ((g_rtc_fg_soc != 0) &&(g_boot_reason == BR_WDT_BY_PASS_PWK || g_boot_reason == BR_WDT||g_boot_reason == BR_RTC)))
    ...
    {
    	if(((g_rtc_fg_soc != 0) && ((gFG_capacity_by_v-g_rtc_fg_soc) < 40) &&(( gFG_capacity_by_v > 1 || bat_is_charger_exist() == KAL_TRUE)))
			|| ((g_rtc_fg_soc != 0) &&(g_boot_reason == BR_WDT_BY_PASS_PWK || g_boot_reason == BR_WDT||g_boot_reason == BR_RTC)))
		{
            gFG_capacity_by_v = g_rtc_fg_soc;            
        }
    }  
//pengjinlong  add for recovery mode JELY-1572	(end)      
//pengjinlong change (DELL-888)	(end)
...
void oam_init(void)
{
	...
	static int check_boot = 0;	
	vol_bat = 5; //set avg times
    ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_BAT_SENSE, &vol_bat);
    ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &gFG_voltage);
//pengjinlong  add for recovery mode JELY-1572(start)
//fasttime power on device
	if( gFG_voltage < vol_bat)
	{
		gFG_voltage = vol_bat +50;
		check_boot = 1;
	}
//pengjinlong  add for recovery mode JELY-1572(end)		
//pengjinlong add plug USB ,gFG_voltage would be more high (start)
	if( ((get_boot_mode()==8) || (get_boot_mode()==9) )&&(check_boot == 0))
	{
		gFG_voltage  = vol_bat +20;
	}
//pengjinlong add plug USB ,gFG_voltage would be more high (end)
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage - 25);//pengjinlong -25 for fast read capacity
...
//    {
    	...
    	if(((gFG_capacity_by_v == 100) && (vbat_capacity < 90)) ||(abs((gFG_capacity_by_v-vbat_capacity)>30)))	
    	{
...
kal_int32 get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
    ...
    kal_int32 vbat_thd = 3500;
    kal_int32 ret_time = 600;

    vbat_val = g_sw_vbat_temp;    
    
    if(vbat_val > vbat_thd)
        ret_time = 90*60;	// 90 min
    else
        ret_time = 10*60;	// 10 min
    ...
    kal_int32 car_sleep=0;
    kal_int32 car_wakeup=0;
    kal_int32 last_time=0;
...
kal_int32 battery_meter_get_battery_OCV(void)
{
    int ret=0;
    int val=5;
...
//    kal_int32 vbat_diff = 200;
...
#endif        
    {
        ...
        bm_print(BM_LOG_CRTI, "sleeptime=(%d)s, be_ocv=(%d), af_ocv=(%d), D0=(%d), car1=(%d), car2=(%d) \n",
    		rtc_time_after_sleep - g_rtc_time_before_sleep, g_hw_ocv_before_sleep, hw_ocv_after_sleep,gFG_DOD0, oam_car_1, oam_car_2);
    }
#endif		
