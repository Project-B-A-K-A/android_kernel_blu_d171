...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
    ...
    {   
        //reduce after xxs
		if(timer_counter == 2)//(SYNC_TO_REAL_TRACKING_TIME/BAT_TASK_PERIOD))//60s -> 30s update UI Percentage
		{
			BMT_status.UI_SOC--;
			timer_counter = 0;
		}
...
{ 
	kal_uint32 bat_vol, charger_vol, Vsense, SOC, ZCV; 
	kal_int32 ICharging, temperature, temperatureR, temperatureV;
	...
	static kal_bool  battery_fast_initilized = KAL_TRUE;
//fast read battery voltage(start)	
	if (battery_fast_initilized){
		bat_vol = (battery_meter_get_battery_OCV() - 25);
		battery_fast_initilized  =KAL_FALSE;
	}
	else{
	bat_vol = battery_meter_get_battery_voltage();
	}
//fast read battery voltage(end)	
...
//delete battery  average method
	BMT_status.temperature = temperature;//mt_battery_average_method(&batteryTempBuffer[0],temperature, &temperature_sum, batteryIndex);
...
{	
    ...
    #endif    
/*pengjinlong add for recharging depend on battery temperature start*/	
...
/*pengjinlong add for recharging depend on battery temperature end*/
#endif
...
//pengjinlong add this for battery temp (start)
static void mt_battery_notify_VBatTemp_low_check(void)
{
#if defined(BATTERY_NOTIFY_CASE_0002_VBATTEMP)
    ...
    battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] mt_battery_notify_VBatTemp_low_check (%x)\n", g_BatteryNotifyCode);
#endif        
}
//pengjinlong add this for battery temp (end)
...
void mt_battery_notify_check(void)
{
	...
	if(g_BN_TestMode == 0x0000)	/* for normal case */
    {
		...
		mt_battery_notify_VBatTemp_low_check();
...
void check_battery_exist(void)
{
    ...
    if( baton_count >= 3)
    {
        ...
        else
        {
           ...
           // arch_reset(0,NULL);     //pengjinlong disable ,device will reboot when plug in usb without  battery 
        }
    }    
#endif
}
...
static int battery_resume(struct platform_device *dev)
{
	...
	{
		battery_xlog_printk(BAT_LOG_CRTI, "******** battery driver resume by pcm timer!! ********\n" );
		mutex_lock(&bat_mutex);
		BAT_thread();
		mutex_unlock(&bat_mutex);
	}
